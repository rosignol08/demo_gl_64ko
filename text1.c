...
static GLuint _pId_texte1 = 0;
static GLuint _quad_texte1 = 0;
...
{   
    ...
    case GL4DH_FREE:
        ...
        // if(_quad_texte1) {
        //   glDeleteVertexArrays(1, &_quad_texte1);
        //   _quad_texte1 = 0;
        // }
        if (_pId_texte1)
        {
...
static void init(void)
{
    ...
    _pId_texte1 = gl4duCreateProgram("<vs>shaders/credits.vs", "<fs>shaders/credits.fs", NULL);
    gl4duGenMatrix(GL_FLOAT, "modelViewMatrix");
    gl4duGenMatrix(GL_FLOAT, "projectionMatrix");
    _quad_texte1 = gl4dgGenQuadf();

    // les texte
    initText(&_textTexId1,
             "Il y existe une porte,\n"
             "que des carpes tentent de franchir,\n"
...
static void draw(void)
{
    ...
    glUseProgram(_pId_texte1);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, _textTexId1);

    glUniform1i(glGetUniformLocation(_pId_texte1, "inv"), 1);
    glUniform1i(glGetUniformLocation(_pId_texte1, "tex"), 0);
    // glUniform1f(glGetUniformLocation(_pId_texte1, "time"), t);

    gl4duBindMatrix("modelViewMatrix");
    gl4duLoadIdentityf();

    // Position les crédits au centre
    gl4duScalef(01.0, 0.50, 1.0);
    gl4duTranslatef(0.0, 0.10, -2.30);

    gl4duSendMatrices();
    gl4dgDraw(_quad_texte1);
    // printf("t = %f\n", t);
    glUseProgram(0);
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
static void initText(GLuint *ptId, const char *text)
{
    ...
    if (!s) {
        SDL_FreeSurface(d);
        TTF_CloseFont(font);
        fprintf(stderr, "Erreur lors de SDL_CreateRGBSurface: %s\n", SDL_GetError());
        return;
      }
    assert(s);
