...
static GLuint _pId_text4 = 0;
static GLuint _quad_text4 = 0;
...
void texte4(int state) {
  ...
  case GL4DH_FREE:
    ...
    if(_quad_text4) {
      glDeleteVertexArrays(1, &_quad_text4);
      _quad_text4 = 0;
    }
    if(_pId_text4) {
      glDeleteProgram(_pId_text4);
      _pId_text4 = 0;
    }
...
static void init(void){
  ...
  _pId_text4 = gl4duCreateProgram("<vs>shaders/credits.vs", "<fs>shaders/credits.fs", NULL);
  gl4duGenMatrix(GL_FLOAT, "modelViewMatrix");
  gl4duGenMatrix(GL_FLOAT, "projectionMatrix");
  _quad_text4 = gl4dgGenQuadf();
...
static void draw(void) {
  ...
  t0 = t;
    ...
    glUseProgram(_pId_text4);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, _textTexId1);  
    
    glUniform1i(glGetUniformLocation(_pId_text4, "inv"), 1);
    glUniform1i(glGetUniformLocation(_pId_text4, "tex"), 0);
    glUniform1f(glGetUniformLocation(_pId_text4, "time"), t);
    
    gl4duBindMatrix("modelViewMatrix");
    gl4duLoadIdentityf();
    
    // Position les crédits au centre avec une légère animation 
    gl4duScalef(1.0, 0.40, 1.0);
    gl4duTranslatef(0.0, 0.10, -2.30);
    
    gl4duSendMatrices();
    gl4dgDraw(_quad_text4);
    //printf("t = %f\n", t);
    glUseProgram(0);
}
