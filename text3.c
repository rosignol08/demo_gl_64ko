...
static GLuint _pId_text3 = 0;
static GLuint _quad_text3 = 0;
...
void texte3(int state) {
  ...
  case GL4DH_FREE:
    ...
    if(_quad_text3) {
      glDeleteVertexArrays(1, &_quad_text3);
      _quad_text3 = 0;
    }
    if(_pId_text3) {
      glDeleteProgram(_pId_text3);
      _pId_text3 = 0;
    }
...
static void init(void){
  ...
  _pId_text3 = gl4duCreateProgram("<vs>shaders/credits.vs", "<fs>shaders/credits.fs", NULL);
  gl4duGenMatrix(GL_FLOAT, "modelViewMatrix");
  gl4duGenMatrix(GL_FLOAT, "projectionMatrix");
  _quad_text3 = gl4dgGenQuadf();
  
    //les texte
    initText(&_textTexId1,
        "À bout de forces, elle pria le fleuve.\n"
        "L’eau devint dorée.\n"
        "Elle s’élança une dernière fois."
      );
  /*
...
static void draw(void) {
  ...
  t0 = t;
    ...
    glUseProgram(_pId_text3);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, _textTexId1);  
    
    glUniform1i(glGetUniformLocation(_pId_text3, "inv"), 1);
    glUniform1i(glGetUniformLocation(_pId_text3, "tex"), 0);
    glUniform1f(glGetUniformLocation(_pId_text3, "time"), t);
    
    gl4duBindMatrix("modelViewMatrix");
    gl4duLoadIdentityf();
    
    // Position les crédits au centre avec une légère animation 
    gl4duScalef(01.0, 0.60, 1.0);
    gl4duTranslatef(0.0, 0.10, -2.30);
    
    gl4duSendMatrices();
    gl4dgDraw(_quad_text3);
    //printf("t = %f\n", t);
    glUseProgram(0);
}
