 ...
 */
 ...
 void init(void) {
   ...
   /*quad pour le post-processing */
   _quad = gl4dgGenQuadf();
   _pId_normal = gl4duCreateProgram("<vs>shaders/basic.vs", "<fs>shaders/basic.fs", NULL);
   ...
   /* recup les dimensions actuelles de la fenêtre */
   int w, h;
 ...
 void draw(void) {
   ...
   /*rendu de la scène dans une texture */
   glBindFramebuffer(GL_FRAMEBUFFER, _fbo);
   glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _texId[0], 0);
   
   /* le z-buffer pour le rendu 3D */
   glEnable(GL_DEPTH_TEST);
   
   /* on active le rendu en mode plein */
