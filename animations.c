...
void fondui(void (* a0)(int), void (* a1)(int), Uint32 t, Uint32 et, int state) {
  ...
  case GL4DH_INIT:
    ...
    srand(42);
    GLfloat * map = gl4dmTriangleEdge(vp[2], vp[3], 0.5f);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, vp[2], vp[3], 0, GL_RED, GL_FLOAT, map);
    free(map);
...
//transition en pixel fait par du bruit
void pixels(void (* a0)(int), void (* a1)(int), Uint32 t, Uint32 et, int state) {
  /* INITIALISEZ VOS VARIABLES */
  int vp[4], i;
  GLint tId;
  static GLuint tex[3], pId;
