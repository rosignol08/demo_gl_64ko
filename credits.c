...
static GLuint _pId_credits = 0;
static GLuint _quad_credits = 0;
...
void credits(int state)
{
  ...
  case GL4DH_FREE:
    ...
    if (_quad_credits)
    {
      //gl4dgDelete(_quad_credits);
      _quad_credits = 0;
    }
    if (_pId_credits)
    {
      glDeleteProgram(_pId_credits);
      _pId_credits = 0;
    }
...
static void init(void)
{
  ...
  _pId_credits = gl4duCreateProgram("<vs>shaders/credits.vs", "<fs>shaders/credits.fs", NULL);
  gl4duGenMatrix(GL_FLOAT, "modelViewMatrix");
  gl4duGenMatrix(GL_FLOAT, "projectionMatrix");
  _quad_credits = gl4dgGenQuadf();
...
static void draw(void)
{
  ...
  glUseProgram(_pId_credits);
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, _textTexId1);

  glUniform1i(glGetUniformLocation(_pId_credits, "inv"), 1);
  glUniform1i(glGetUniformLocation(_pId_credits, "tex"), 0);
  glUniform1f(glGetUniformLocation(_pId_credits, "time"), t);
